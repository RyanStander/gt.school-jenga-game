    ...
    public class ApiInterpreter
    {
        ...
        public IEnumerable<Standard> GetStandards()
        {
            ...
            foreach (var line in lines)
            {
                ...
                standard.Id = int.Parse(lineParts[0][(lineParts[0].IndexOf(':') + ColonModifier)..lineParts[0].IndexOf(',')]);

                string subjectString = lineParts[1][(lineParts[1].IndexOf(':') + QuoteModifier)..lineParts[1].LastIndexOf('"')];
                standard.Subject = (Subject)System.Enum.Parse(typeof(Subject), subjectString, true);

                standard.Grade = lineParts[2][(lineParts[2].IndexOf(':') + QuoteModifier)..lineParts[2].LastIndexOf('"')];
                standard.Mastery = int.Parse(lineParts[3][(lineParts[3].IndexOf(':') + ColonModifier)..lineParts[3].IndexOf(',')]);
                standard.DomainId = lineParts[4][(lineParts[4].IndexOf(':') + QuoteModifier)..lineParts[4].LastIndexOf('"')];
                standard.DomainName = lineParts[5][(lineParts[5].IndexOf(':') + QuoteModifier)..lineParts[5].LastIndexOf('"')];
                standard.ClusterDescription =
                    lineParts[6][(lineParts[6].IndexOf(':') + QuoteModifier)..lineParts[6].LastIndexOf('"')];
                standard.StandardId = lineParts[7][(lineParts[7].IndexOf(':') + QuoteModifier)..lineParts[7].LastIndexOf('"')];
